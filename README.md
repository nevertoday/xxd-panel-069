...
原始 X 样张及其可核验出处尚未提供，因此 `sample-01`–`sample-04` 暂不展示。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 069 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

很多照片转成插画后，只剩一块柔软水彩色斑：画面变轻了，主体却被“套框”，笔触、留白和文字彼此没有关系。

**Panel 069** 解决的是另一件事。它把照片中最值得记住的主体和关系，放进由大号粗笔真实刷出的不规则绘画视窗；宽厚色块负责建立方向与版式，纤细描边负责保住身份，少量厚涂和鲜活取色让画面保持生命力。主体还能自然越过笔触边界，使视窗、留白和文字成为一个完整设计，而不是照片蒙版。

### 适合这些情况

- **主体必须可认：** 人物、建筑、动物、植物、器物或风景的身份不能被通用色斑替换。
- **想要鲜活而不廉价：** 从照片最明亮、最有生命力的颜色重新调制，不做灰脏滤镜或糖果色堆叠。
...
> 用 XXD Panel 069 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 粗笔绘画视窗 · 鲜活取色 · 纤细描边 · 越界关系 · 大量暖白留白 · 自由图文混排

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 069 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 把照片重构成粗笔色块与纤细线条共同组织的明亮编辑画面 |
| **一眼可辨的特点** | 大号刷痕形成不规则视窗，主体从色块中显现并自然越界 |
| **它如何尊重你的输入** | 保留主体身份、姿态和关键关系；颜色从照片最鲜活的部分重新调制，不使用外层固定色板 |
...
```text
锁定主体身份与叙事关系 → 删除不必要背景 → 找到原图最有美感的方向 → 用数笔宽厚刷痕建立不规则视窗 → 以纤细描边和少量色块让主体显形 → 让关键轮廓自然越界 → 从最鲜活的源图颜色重新调制 → 在暖白留白中完成克制图文编排
```

## 成品中最容易识别的风格特点

- 绘画视窗必须看得出由大号粗笔直接刷成，保留笔毛、干刷断裂、厚薄和毛躁边缘。
- 视窗不是水彩晕染框、规整几何框或照片蒙版，本身就是版式中的主形体。
- 主体以少量关键形体保持一眼可认，并通过植物、人物、尖顶或轮廓越过笔触边界。
- 粗块面与纤细描边形成明确尺度反差，焦点位置可加入少量真实厚涂。
- 色彩只从原图最明亮、最鲜活、最有生命力的部分推导，以暖白留白保持呼吸。
- 文字从当前照片的主体、地点、情绪、动作或隐喻生长，并与刷痕方向和越界形体呼应。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-069-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 069 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-069 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-069 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-069 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 069；公共设置只解析一次
/xxd-panel-069 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-069`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-069-prompt.en.md) · [中文运行适配器](references/xxd-panel-069-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全系列项目

69 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–069 的全部项目地址和核心风格特点；当前项目以粗体标出。
