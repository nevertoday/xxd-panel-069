...
`sample-01`–`sample-04` remain reserved for verifiable original release samples. The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 069 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

Many photo-to-illustration workflows collapse into a soft watercolour blot: the result looks lighter, but the subject is merely placed inside a decorative mask and the brushwork, whitespace, and typography do not meaningfully interact.

**Panel 069** builds a different relationship. A real broad-brush window establishes direction and editorial structure; fine hand-drawn contours preserve identity; selected impasto and the source's most vital colours keep the work alive. Defining forms may cross beyond the brush boundary, binding painted mass, whitespace, subject, and copy into one art direction.

### Best for

- recognisable people, architecture, animals, plants, objects, vehicles, and landscapes;
- vivid, restorative colour without dirty filters or cheap candy saturation;
- editorial layouts where broad paint, bright space, boundary crossings, and sparse type share one rhythm;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery.

### Quick start

> Use XXD Panel 069 on this image and recommend the most suitable composition and size first.
...
**Signature:** broad-brush painted window · vital source colour · fine contours · boundary crossings · warm-white space · free image–text composition
...
|---|---|
| **What you get** | A bright editorial artwork organised by broad colour masses and fine hand-drawn lines |
| **Recognisable signature** | An irregular window visibly brushed with a large tool, with the subject emerging from and crossing its edges |
| **How it respects the source** | Identity, pose, and key relationships remain recognisable; colour is remixed from the photograph's most vital hues rather than an outer fixed palette |
...
```text
lock identity and narrative relation → remove secondary background → find the source's strongest direction → establish an irregular window with several broad strokes → reveal the subject through fine contours and selected colour → let defining forms cross the brush boundary → remix the most vital source colours → compose sparse type in warm-white space
```

## Recognisable finished traits

- The window visibly comes from a large brush, retaining bristles, dry breaks, thickness variation, rough edges, and exposed white.
- It is a compositional shape—not a soft watercolour halo, tidy frame, or photographic mask.
- A few key forms preserve recognition while plants, figures, spires, or contours cross the painted edge.
- Broad masses and fine contours create a deliberate scale contrast, with restrained impasto at focal points.
- Colour comes from the source's brightest and most vital relations, balanced by generous warm-white breathing space.
- Copy grows from the current photograph and answers the brush direction or boundary-crossing forms.

All aesthetic constraints live only in the [canonical prompt](references/original-prompt/zh-CN.md). The Skill and runtime adapters resolve delivery variables. [Skill workflow](SKILL.md) · [English runtime adapter](references/xxd-panel-069-prompt.en.md)
...
```text
/xxd-panel-069 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-069 photo.jpg --prefs last
/xxd-panel-069 photo.jpg --prefs edit
/xxd-panel-069 photo.jpg --prefs new
```
...
/xxd-panel-069 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-069 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-069 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-069 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-069 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-069 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-069 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-069 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-069 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-069 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-069 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 069 to a whole directory; shared settings resolve once
/xxd-panel-069 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-069 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-069.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-069" ~/.codex/skills/xxd-panel-069
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-069 --skill xxd-panel-069
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-069`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-069-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-069-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## Complete XXD Panel catalog

Each of the 69 Panels preserves its own original brief and aesthetic logic. The table continuously lists every project from 001 through 069 with its defining style characteristics; the current project is bold.
...
| **Soldiers** | `xxd-panel-NNN` (currently 001–069) | Each numbered Skill executes only its own original brief and aesthetic, completing the individual job assigned by the General. |
