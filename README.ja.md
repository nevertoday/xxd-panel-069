...
```text
同一性と関係を固定 → 不要背景を削除 → 最も美しい方向を特定 → 数本の粗筆で不規則なウィンドウを構築 → 繊細な輪郭と選択的な色で顕在化 → 特徴を自然に越境 → 元写真の最も鮮活な色を再調整 → 暖白の余白に文字を編成
```

## 完成作の特徴

- 筆毛、ドライブラシの途切れ、厚薄、荒い縁、白抜けから大筆の実在感が見える。
- 水彩のにじみ枠や写真マスクではなく、ウィンドウ自体が版面の主形体になる。
- 少数の形で認識性を守り、植物・人物・尖塔・輪郭が筆触の縁を越える。
- 粗い面と繊細な輪郭に明確な尺度差があり、焦点だけ少量の厚塗りを使う。
- 元写真の最も明るく生命力ある色を、暖白の大きな余白で支える。
- 文字は現在の写真から生まれ、筆触方向や越境形体と呼応する。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-069-prompt.en.md)
