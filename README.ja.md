...
# 🦁 XXD Panel 069｜鮮やか塗りの窓

### 粗筆の絵画ウィンドウ、鮮やかな色、繊細な輪郭線で写真を再構成する
...
検証可能なオリジナル公開作例と出典が未提供のため、`sample-01`–`sample-04` は掲載していません。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 069 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

一般的な水彩フレームでは、写真が柔らかな色斑の中に置かれるだけで、筆触、余白、文字が別々に見えがちです。

**Panel 069** は、大きな筆で実際に刷った不規則なウィンドウを構図の主形体にします。太い色面が方向と版面をつくり、繊細な輪郭線が被写体の同一性を守り、元写真の最も生命力ある色と少量の厚塗りが鮮やかさを支えます。特徴的な形は筆触の外へ自然に越境し、色面、余白、被写体、文字を一つのアートディレクションへ結びます。

### 向いている用途

- 被写体を一目で認識できるまま、写真を編集的な絵画へ変換したい場合
- 濁ったフィルターや安価なキャンディ色ではなく、明るく生命感のある色を求める場合
- 粗い色面、明るい余白、越境する輪郭、少量の文字を一つのリズムにしたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 069 で処理し、最適な構図とサイズを先に提案してください。
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 太い色面と繊細な手描き線で構成された明るい編集画面 |
| **識別しやすい特徴** | 大きな筆で刷った不規則なウィンドウから被写体が現れ、特徴が縁を越える |
| **入力の尊重** | 同一性、姿勢、関係を維持し、色は固定パレットではなく写真の最も生命力ある部分から再調整する |
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-069`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-069-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-069-prompt.zh-CN.md)
...
**特徴：** 粗筆ウィンドウ · 鮮活な元画像色 · 繊細な輪郭 · 越境関係 · 暖白の余白 · 自由な図文混排
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 069 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-069 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-069 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-069 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-069 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 069 を適用し、共通設定は一度だけ解決
/xxd-panel-069 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
69 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 069 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
