...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 069 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 069 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 069 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 069 Sample 8"></td>
  </tr>
</table>
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 069 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 069 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 069 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 069 additional top-bottom sample 4"></td>
  </tr>
</table>
...
검증 가능한 원본 공개 예시와 출처가 없어 `sample-01`–`sample-04`는 현재 표시하지 않습니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 069이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

일반적인 수채 프레임은 사진을 부드러운 색 얼룩 안에 넣는 데 그쳐 붓질, 여백, 글자가 서로 따로 보이기 쉽습니다.

**Panel 069**는 큰 붓으로 직접 칠한 불규칙한 회화 창을 구도의 핵심 형태로 사용합니다. 넓은 색면은 방향과 편집 질서를 만들고, 섬세한 윤곽선은 주체의 정체성을 지키며, 원본의 가장 생명력 있는 색과 소량의 두꺼운 물감은 화면을 생생하게 만듭니다. 핵심 형태는 붓질 경계를 자연스럽게 넘어 색면, 여백, 주체, 문구를 하나의 아트 디렉션으로 연결합니다.

### 이런 경우에 적합합니다

- 인물·건축·동물·식물·사물·풍경의 정체성을 유지한 채 편집 회화로 바꾸고 싶을 때
- 탁한 필터나 값싼 사탕색이 아닌 밝고 생명력 있는 색을 원할 때
- 굵은 색면, 밝은 여백, 경계를 넘는 윤곽, 적은 글자를 하나의 리듬으로 구성할 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 069로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 굵은 색면과 섬세한 손그림 선이 함께 구성한 밝은 편집 화면 |
| **대표 특징** | 큰 붓으로 칠한 불규칙한 창에서 주체가 드러나고 핵심 특징이 경계를 넘음 |
| **입력 존중 방식** | 정체성, 자세, 관계를 유지하고 외부 고정 팔레트가 아니라 사진의 가장 생명력 있는 색을 재조율함 |
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-069`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-069-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-069-prompt.zh-CN.md)
...
**특징:** 굵은 붓의 회화 창 · 생생한 원본 색 · 섬세한 윤곽 · 경계 넘기 · 웜 화이트 여백 · 자유로운 이미지·텍스트 편집
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 069의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-069 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-069 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-069 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-069 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-069 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-069 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 069 적용, 공통 설정은 한 번만 확정
/xxd-panel-069 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
69개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 069까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
