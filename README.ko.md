...
```text
정체성과 관계 고정 → 불필요한 배경 삭제 → 가장 아름다운 방향 찾기 → 몇 번의 굵은 붓질로 불규칙한 창 구성 → 섬세한 윤곽과 선택적 색으로 주체 드러내기 → 핵심 특징을 자연스럽게 경계 밖으로 연결 → 원본의 가장 생생한 색 재조율 → 웜 화이트 여백에 문구 편집
```

## 완성작의 특징

- 붓털, 드라이브러시 단절, 두께 변화, 거친 가장자리, 흰 바탕 노출로 큰 붓의 실재감이 보입니다.
- 수채 번짐 프레임이나 사진 마스크가 아니라 회화 창 자체가 편집 구도의 핵심 형태입니다.
- 소수의 형태로 인식성을 지키고 식물·인물·첨탑·윤곽이 붓질 경계를 넘습니다.
- 굵은 면과 섬세한 윤곽의 크기 대비가 분명하고 초점에만 소량의 두꺼운 물감을 사용합니다.
- 원본의 가장 밝고 생명력 있는 색을 넓은 웜 화이트 여백이 받칩니다.
- 문구는 현재 사진에서 나오며 붓질 방향과 경계를 넘는 형태에 호응합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-069-prompt.en.md)
